...
import DevPasswordGate from './dev/DevPasswordGate';
...
export default function Footer() {
  ...
  const { isDevMode, openPanel } = useDevMode();
  ...
  return (
    <>
      {showGate && (
        <DevPasswordGate onClose={() => setShowGate(false)} />
      )}
      <footer className="bg-[#1A1A1A] border-t-2 border-[#F5F0E8]/10 py-10 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <button
            onClick={handleLogoClick}
            className="flex items-center gap-2 font-mono font-bold text-[#F5F0E8] select-none"
          >
